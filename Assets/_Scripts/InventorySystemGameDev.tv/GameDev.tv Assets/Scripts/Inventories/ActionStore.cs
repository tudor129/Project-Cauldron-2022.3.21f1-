...
namespace GameDevTV.Inventories
{
    ...
    public class ActionStore : MonoBehaviour, ISaveable
    {
        ...
        /// </summary>
        public ActionItemSo GetAction(int index)
        {
            if (_dockedItems.ContainsKey(index))
            {
                return _dockedItems[index].ItemSo;
            }
            return null;
        }
        ...
        /// </summary>
        /// <param name="itemSo">What item should be added.</param>
        /// <param name="index">Where should the item be added.</param>
        /// <param name="number">How many items to add.</param>
        public void AddAction(ItemSO itemSo, int index, int number)
        {
            if (_dockedItems.ContainsKey(index))
            {  
                if (object.ReferenceEquals(itemSo, _dockedItems[index].ItemSo))
                {
            ...
            else
            {
                ...
                slot.ItemSo = itemSo as ActionItemSo;
                slot.number = number;
        ...
        public bool Use(int index, GameObject user)
        {
            if (_dockedItems.ContainsKey(index))
            {
                _dockedItems[index].ItemSo.Use(user);
                if (_dockedItems[index].ItemSo.isConsumable())
                {
                    RemoveItems(index, 1);
                }
                return true;
        ...
        public int MaxAcceptable(ItemSO itemSo, int index)
        {
            var actionItem = itemSo as ActionItemSo;
            if (!actionItem) return 0;

            if (_dockedItems.ContainsKey(index) && !object.ReferenceEquals(itemSo, _dockedItems[index].ItemSo))
            {
                return 0;
            }
        ...
        object ISaveable.CaptureState()
        {
            ...
            foreach (var pair in _dockedItems)
            {
                ...
                record.itemID = pair.Value.ItemSo.GetItemID();
        ...
        void ISaveable.RestoreState(object state)
        {
            ...
            {
                AddAction(ItemSO.GetFromID(pair.Value.itemID), pair.Key, pair.Value.number);
            }
        }
    }
}